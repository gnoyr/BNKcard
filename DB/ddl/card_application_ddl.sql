...
CREATE TABLE CREDIT_CARD_APPLICATIONS (
    ...
    payment_snapshot            CLOB,   -- { card_brand, payment_day, combined_transit_yn, tx_alert_type, statement_method, overseas_dcc_block_yn }
...
COMMENT ON COLUMN CREDIT_CARD_APPLICATIONS.payment_snapshot            IS '신청정보 JSON. card_brand/payment_day/combined_transit_yn/tx_alert_type/statement_method/overseas_dcc_block_yn';
...
COMMENT ON COLUMN CHECK_CARD_APPLICATIONS.payment_snapshot             IS '신청정보 JSON. card_brand/payment_day/combined_transit_yn/tx_alert_type/statement_method/overseas_dcc_block_yn';
...
COMMENT ON COLUMN USER_CARDS.card_password_hash IS '카드 결제 비밀번호 BCrypt 해시값. 발급 시 서비스에서 BCrypt 후 저장. 비밀번호 변경 시 업데이트';
