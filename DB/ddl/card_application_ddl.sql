...
/
BEGIN EXECUTE IMMEDIATE 'ALTER TABLE USER_CARDS RENAME COLUMN version_id TO card_id'; EXCEPTION WHEN OTHERS THEN NULL; END;
/
BEGIN EXECUTE IMMEDIATE 'ALTER TABLE USER_CARDS RENAME COLUMN card_number TO masked_card_number'; EXCEPTION WHEN OTHERS THEN NULL; END;
/
BEGIN EXECUTE IMMEDIATE 'ALTER TABLE USER_CARDS DROP CONSTRAINT FK_USER_CARDS_VERSION'; EXCEPTION WHEN OTHERS THEN NULL; END;
/
BEGIN EXECUTE IMMEDIATE 'ALTER TABLE USER_CARDS DROP CONSTRAINT FK_USER_CARDS_CREDIT_APP'; EXCEPTION WHEN OTHERS THEN NULL; END;
/
BEGIN EXECUTE IMMEDIATE 'ALTER TABLE USER_CARDS DROP CONSTRAINT FK_USER_CARDS_CHECK_APP'; EXCEPTION WHEN OTHERS THEN NULL; END;
...
CREATE TABLE CREDIT_CARD_APPLICATIONS (
    ...
    ci_value                    VARCHAR2(200),  -- 경찰청 본인확인 후 수신한 CI값 (AES 암호화)
...
COMMENT ON COLUMN CREDIT_CARD_APPLICATIONS.estimated_monthly_income    IS '사용자 전체 신고소득 기반 추정 월소득. 심사서버에서 받음';
...
-- 불필요한 컬럼 제거
ALTER TABLE USER_CARDS DROP COLUMN card_nickname;
ALTER TABLE USER_CARDS DROP COLUMN daily_limit_amount;
ALTER TABLE USER_CARDS DROP COLUMN monthly_limit_amount;
ALTER TABLE USER_CARDS DROP COLUMN issued_by;

-- 신청 참조 컬럼 및 비밀번호 해시 추가
...
COMMENT ON COLUMN USER_CARDS.card_password_hash IS '카드 결제 비밀번호 BCrypt 해시값';
