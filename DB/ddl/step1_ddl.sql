...
ALTER TABLE CARD_EVENT_LOGS MODIFY (error_message VARCHAR2(2000));
ALTER TABLE TERMS_EVENT_LOGS MODIFY (error_message VARCHAR2(2000));
ALTER TABLE CHAT_EVENT_LOGS MODIFY (error_message VARCHAR2(2000));
...
-- USER TABLE 데이터삭제  -------------------------------
-- ① EVENT_LOGS 자식 테이블
DELETE FROM CARD_EVENT_LOGS;
DELETE FROM TERMS_EVENT_LOGS;
DELETE FROM CHAT_EVENT_LOGS;
DELETE FROM EVENT_LOGS;

-- ② 계좌
DELETE FROM ACCOUNT_PASSWORDS;
...
DELETE FROM TERMS_NOTIFICATION_HISTORY;
DELETE FROM USER_CARDS;
DELETE FROM USER_CDD_CHECKS;
DELETE FROM USER_PASSWORD_HISTORIES;
DELETE FROM USER_SESSIONS;
DELETE FROM USER_SPENDING_PATTERNS;
...
-- audit_logs 데이터 삭제~~~
-- 1. 트리거 비활성화
ALTER TRIGGER TRG_AUDIT_LOGS_NO_DEL DISABLE;
...
-- 3. 트리거 다시 활성화
ALTER TRIGGER TRG_AUDIT_LOGS_NO_DEL ENABLE;
