-- ================================================================
-- BNK 부산은행 금융 상품 플랫폼 — 전체 DDL (Oracle 21c)
-- ※ Ctrl+A → 전체 실행 안전 버전
--   [SECTION 1] DROP  : 트리거 → 함수 → 테이블 → 시퀀스 순으로 전부 삭제
--   [SECTION 2] CREATE: 시퀀스 → 함수 → 테이블 → 트리거 → 인덱스 순으로 재생성
--
-- [수정 이력]
--   2026-06-15: CARD_APPLICATIONS 관련 코드 주석 처리
--               → 대체 테이블: CREDIT_CARD_APPLICATIONS, CHECK_CARD_APPLICATIONS
--               → 신청 테이블은 card_application_ddl.sql에서 별도 관리
--               → 실행 순서: busanbank_ddl.sql → card_application_ddl.sql
-- ================================================================


-- ================================================================
-- [SECTION 1] FULL DROP  (IF-EXISTS 효과: EXCEPTION WHEN OTHERS THEN NULL)
-- ================================================================
...
/
-- [주석 처리] CARD_APPLICATIONS 대체됨 → card_application_ddl.sql 참고
...
-- ── 1-3. 테이블 DROP (CASCADE CONSTRAINTS PURGE — FK 순서 무관) ──
...
CREATE TABLE CARDS (
    ...
    view_count                  NUMBER(10)     DEFAULT 0 NOT NULL CHECK (view_count >= 0),
    ...
    deleted_yn                  CHAR(1)        DEFAULT 'N' CHECK (deleted_yn IN ('Y','N')),
    deleted_at                  TIMESTAMP,
    CONSTRAINT CHK_CARDS_MAX_AGE        CHECK (maximum_age IS NULL OR maximum_age >= minimum_age),
    CONSTRAINT CHK_CARDS_CREDIT_MAX     CHECK (credit_limit_max IS NULL OR credit_limit_max >= credit_limit_min),
    CONSTRAINT CHK_CARDS_PUBLISH_DATE   CHECK (publish_end_at IS NULL OR publish_end_at >= publish_start_at)
);
...
-- [주석 처리] CREDIT_CARD_APPLICATIONS / CHECK_CARD_APPLICATIONS 로 대체됨
-- → card_application_ddl.sql 에서 생성
-- ================================================================
/*
...
-- ================================================================
-- 27. USER_CARDS
-- [수정] application_id 제거
--       credit_app_id, check_app_id 는 card_application_ddl.sql에서 ALTER로 추가
-- ================================================================
CREATE TABLE USER_CARDS (
    user_card_id            NUMBER(19)     PRIMARY KEY,
    user_id                 NUMBER(19)     NOT NULL,
    card_id                 NUMBER(10)     NOT NULL,
    -- [주석 처리] CARD_APPLICATIONS 대체됨 → card_application_ddl.sql에서 credit_app_id/check_app_id로 ALTER 추가
...
-- [수정] credit_app_id, check_app_id 는 card_application_ddl.sql에서 ALTER로 추가
-- ================================================================
CREATE TABLE USER_TERMS_AGREEMENTS (
    ...
    ip_address              VARCHAR2(100),
    user_agent              VARCHAR2(1000),
    -- [주석 처리] card_application_ddl.sql에서 ALTER로 추가됨
...
COMMENT ON COLUMN USER_TERMS_AGREEMENTS.agreement_source   IS 'SIGNUP / CREDIT_CARD_APPLY / CHECK_CARD_APPLY / EVENT';
...
-- [주석 처리] CARD_APPLICATIONS 대체됨 → card_application_ddl.sql 참고
-- CREATE SEQUENCE SEQ_CARD_APPLICATIONS       START WITH 1   INCREMENT BY 1 NOCACHE NOCYCLE;
...
CREATE SEQUENCE SEQ_CARD_SERIAL_CREDIT         START WITH 1   INCREMENT BY 1 MINVALUE 1 MAXVALUE 999 NOCACHE NOCYCLE;
CREATE SEQUENCE SEQ_CARD_SERIAL_CHECK          START WITH 1   INCREMENT BY 1 MINVALUE 1 MAXVALUE 999 NOCACHE NOCYCLE;
CREATE SEQUENCE SEQ_CARD_SERIAL_PREPAID        START WITH 1   INCREMENT BY 1 MINVALUE 1 MAXVALUE 999 NOCACHE NOCYCLE;
CREATE SEQUENCE SEQ_CARD_SERIAL_HYBRID         START WITH 1   INCREMENT BY 1 MINVALUE 1 MAXVALUE 999 NOCACHE NOCYCLE;
...
-- BEFORE INSERT ON CARD_APPLICATIONS FOR EACH ROW WHEN (NEW.application_id IS NULL)
-- BEGIN :NEW.application_id := SEQ_CARD_APPLICATIONS.NEXTVAL; END TRG_CARD_APPLICATIONS_BI;
-- /
