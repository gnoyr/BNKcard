...
CREATE TABLE CARDS (
    ...
    deleted_yn                  CHAR(1)        DEFAULT 'N' CHECK (deleted_yn IN ('Y','N')),
    deleted_at                  TIMESTAMP,
    CONSTRAINT CHK_CARDS_MAX_AGE
        CHECK (maximum_age IS NULL OR maximum_age >= minimum_age),
    CONSTRAINT CHK_CARDS_CREDIT_MAX
        CHECK (credit_limit_max IS NULL OR credit_limit_max >= credit_limit_min),
...
CREATE SEQUENCE SEQ_TERMS_MASTERS
    START WITH 126 INCREMENT BY 1 NOCACHE NOCYCLE;
...
-- INDEXES
-- ================================================================
-- IDX_USERS_EMAIL: USERS.email은 UNIQUE 제약으로 인덱스 자동 생성됨 (중복 생성 불필요)
CREATE INDEX IDX_USERS_STATUS ON USERS(status_code);
CREATE INDEX IDX_USERS_PHONE ON USERS(phone);
CREATE INDEX IDX_USERS_NAME ON USERS(name);
-- IDX_ADMIN_USERS_USERNAME: ADMIN_USERS.username은 UNIQUE 제약으로 인덱스 자동 생성됨 (중복 생성 불필요)
