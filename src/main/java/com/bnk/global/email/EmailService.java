...
public class EmailService {
	...
	public void sendPasswordResetEmail(String to, String token) {
		...
		send(to, "[BNK카드] 비밀번호 재설정", buildPasswordResetHtml(resetUrl));
	}
	...
	private String buildPasswordResetHtml(String resetUrl) {
		return "<!DOCTYPE html>" + "<html lang='ko'>" + "<head>" + "<meta charset='UTF-8'>"
				...
				"<p style='margin:0 0 28px;color:" + MUTED + ";font-size:14px;line-height:1.7;'>" + "아래 버튼을 클릭하여 비밀번호를 "
				+ "재설정하세요.<br>" + "링크는 요청 후 <strong style='color:" + TEAL_800 + ";'>30분</strong>" + " 동안 유효합니다.</p>" +

				// 버튼
				"<table cellpadding='0' cellspacing='0' border='0'><tr><td>" + "<a href='" + resetUrl + "'"
				...
				+ "text-decoration:none;letter-spacing:0.3px;'>비밀번호 재설정하기</a>" + "</td></tr></table>" +

				"<p style='margin:20px 0 0;color:" + MUTED + ";font-size:12px;line-height:1.8;"
				+ "border-left:3px solid " + TEAL_400 + ";padding-left:12px;'>" + "본인이 요청하지 않은 경우 이 메일을 무시하세요.</p>" +
	...
	@Async
	public void sendDeviceVerifyCode(String to, String code) {
		send(to, "[BNK카드] 새 기기 인증 코드", buildDeviceVerifyHtml(code));
	}

	private String buildDeviceVerifyHtml(String code) {
		return "<!DOCTYPE html>" + "<html lang='ko'>" + "<head>" + "<meta charset='UTF-8'>"
				+ "<meta name='viewport' content='width=device-width,initial-scale=1'>" + "</head>"
				+ "<body style='margin:0;padding:0;background:" + BG + ";"
				+ "font-family:\"Apple SD Gothic Neo\",\"Malgun Gothic\",Arial,sans-serif;'>" +
				...
				// 제목
				"<p style='margin:0 0 6px;color:" + TEAL_800 + ";font-size:20px;font-weight:700;"
				+ "letter-spacing:-0.3px;'>새 기기 인증</p>" +

				// 부제목
				"<p style='margin:0 0 24px;color:" + MUTED + ";font-size:14px;line-height:1.7;'>"
				+ "아래 인증번호를 인증 화면에 입력해 주세요.<br>" + "인증번호는 <strong style='color:" + TEAL_800
	...
	// ──────────────────────────────────────────────────────────────────
	// 통합 알림 이메일 발송 [NotificationService EMAIL 채널]
	// ──────────────────────────────────────────────────────────────────
	@Async
	public void sendNotificationEmail(String to, String title, String message, String linkUrl) {
		send(to, "[BNK카드] " + title, buildNotificationHtml(title, message, linkUrl));
	}

	private String buildNotificationHtml(String title, String message, String linkUrl) {
		String safeTitle = escape(title);
		String safeMessage = escape(message).replace("\n", "<br>");
		boolean hasLink = linkUrl != null && !linkUrl.isBlank();
		String absoluteUrl = hasLink ? (linkUrl.startsWith("http") ? linkUrl : baseUrl + linkUrl) : "";

		String button = hasLink
				? "<table cellpadding='0' cellspacing='0' border='0'><tr><td>" + "<a href='" + absoluteUrl + "'"
						+ " style='display:inline-block;background:" + TEAL_600 + ";color:" + WHITE + ";"
						+ "font-size:15px;font-weight:700;padding:14px 32px;border-radius:8px;"
						+ "text-decoration:none;letter-spacing:0.3px;'>자세히 보기</a>" + "</td></tr></table>"
				: "";
		...
		return "<!DOCTYPE html><html><body style='margin:0;padding:0;background:" + BG + ";'>"
				...
				+ ";padding:32px 0;'>" + "<tr><td align='center'>"
				+ "<table width='480' cellpadding='0' cellspacing='0' border='0'" + " style='background:" + WHITE
				+ ";border-radius:10px;overflow:hidden;"
				+ "box-shadow:0 4px 20px rgba(0,103,127,0.13);max-width:480px;width:100%;'>"
				...
				// ── 본문 ──
				+ "<tr><td style='padding:36px 36px 28px;'>" + "<p style='margin:0 0 14px;color:" + TEAL_800
				+ ";font-size:20px;font-weight:700;letter-spacing:-0.3px;'>" + safeTitle + "</p>"
				+ "<p style='margin:0 0 28px;color:" + MUTED + ";font-size:14px;line-height:1.7;'>" + safeMessage
				+ "</p>" + button + "</td></tr>"

				// ── 푸터 ──
				+ "<tr><td style='background:" + FOOT_BG + ";padding:18px 36px;border-top:1px solid " + TEAL_100 + ";'>"
				+ "<table cellpadding='0' cellspacing='0' border='0' width='100%'><tr>"
				+ "<td><p style='margin:0;color:#9BB4BB;font-size:11px;'>&copy; BNK카드. All rights reserved.</p></td>"
				+ "<td align='right'><p style='margin:0;color:#9BB4BB;font-size:11px;'>부산광역시 동대신구 수정로 60</p></td>"
				+ "</tr></table></td></tr>"
