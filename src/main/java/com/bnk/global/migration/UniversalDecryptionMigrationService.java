...
public class UniversalDecryptionMigrationService {
    ...
    // ══════════════════════════════════════════════════════════════
    // 컬럼 단위 처리 — REQUIRES_NEW 독립 트랜잭션
    // ══════════════════════════════════════════════════════════════
 // @Transactional(propagation = Propagation.REQUIRES_NEW)  ← 제거
    public ColumnResult processColumn(String tableName, String columnName, String pkColumn) {
        ...
        while (offset < total) {
            ...
            for (Map<String, Object> row : batch) {
                ...
                // ★ 모든 테이블 JdbcTemplate으로 직접 UPDATE + 즉시 커밋
                try {
                    if (isLogTable) {
                        updateLogTable(tableName, columnName, pkColumn, pkValue, plainText);
                    } else {
                        // ★ mapper 대신 JdbcTemplate 직접 사용 → 트랜잭션 문제 없음
                        jdbcTemplate.update(
                            "UPDATE " + tableName
                            + " SET " + columnName + " = ?"
                            + " WHERE " + pkColumn + " = ?",
    ...
    // ══════════════════════════════════════════════════════════════
    // 로그 테이블 UPDATE — 트리거 실존 확인 후 DISABLE/UPDATE/ENABLE
    // ══════════════════════════════════════════════════════════════
    private void updateLogTable(String tableName, String columnName,
        ...
        try {
            ...
            jdbcTemplate.update(
                "UPDATE " + tableName + " SET " + columnName + " = ? WHERE " + pkColumn + " = ?",
                plainText, pkValue);
        } finally {
            if (triggerName != null) {
                try {
    ...
    /**
     * 테이블의 UPDATE 차단 트리거가 실제 DB에 존재하는지 확인 후 반환.
     * 존재하지 않으면 null → 트리거 비활성화 없이 일반 UPDATE 진행.
     */
    private String getExistingTriggerName(String tableName) {
        ...
        try {
            Integer count = jdbcTemplate.queryForObject(
                "SELECT COUNT(1) FROM USER_TRIGGERS WHERE TRIGGER_NAME = ?",
