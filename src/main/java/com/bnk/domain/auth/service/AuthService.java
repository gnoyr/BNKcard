...
public class AuthService {
	...
	public Long signup(SignupRequest request) {
		...
		User user = User.builder().email(request.getEmail()).passwordHash(passwordEncoder.encode(request.getPassword()))
				.name(request.getName()).phone(formattedPhone).birthDate(birthDate)
	...
	public AuthTokenResult adminLogin(AdminLoginRequest request, HttpServletRequest httpRequest) {
		...
		if (!passwordEncoder.matches(request.getPassword(), admin.getPasswordHash())) {
			...
			// 관리자 로그인 실패 이력 INSERT
			adminUserMapper.insertLoginHistory("ADMIN", admin.getAdminId(), "FAIL", "비밀번호 불일치", ipAddress, null,
					userAgent);
			throw new BusinessException(ErrorCode.INVALID_PASSWORD);
		}
		...
		// 관리자 로그인 성공 이력 INSERT
		adminUserMapper.insertLoginHistory("ADMIN", admin.getAdminId(), "SUCCESS", null, ipAddress, null, userAgent);

		String roles = admin.getRoleCodes() != null ? String.join(",", admin.getRoleCodes()) : "ADMIN";
	...
	/**
	 * USER_SESSIONS INSERT + 쿠키 생성. ip_address / user_agent 도 세션에 함께 저장.
	 */
	...
	/**
	 * 프록시/로드밸런서 환경을 고려한 실제 클라이언트 IP 추출. X-Forwarded-For 헤더가 있으면 첫 번째 IP를 사용하고, 없으면
	 * RemoteAddr 를 사용한다.
	 */
