...
/**
 * AI 평가 지표 통합 테스트 (실제 Gemini API 호출)
 *
 * [수정 이력]
 * - @MockitoBean ObjectStorageClient 추가 (ObjectStorageConfig @Profile("!test") 비활성)
 * - TestDataSourceConfig 추가: MyBatisConfig가 DataSource 파라미터를 직접 요구
 *   DataSourceAutoConfiguration exclude 시 DataSource 빈 부재 → H2 수동 등록으로 해결
 */
...
class AiChatEvaluationTest {
    ...
    @MockitoBean private ObjectStorageClient objectStorageClient;
    ...
    void tearDown() throws InterruptedException {
        System.out.println("⏳ Rate Limit(15 RPM) 방어 중... 5초 대기합니다.");
        Thread.sleep(5000);
    }
    ...
    void testFaithfulness() {
        FaithfulnessMetric.FaithfulnessConfig config =
            FaithfulnessMetric.FaithfulnessConfig.builder().language("ko").build();
    ...
    void testContextRecall() {
        ContextRecallMetric.ContextRecallConfig config =
            ContextRecallMetric.ContextRecallConfig.builder().language("ko").build();
    ...
    void testContextPrecision() {
        ContextPrecisionMetric.ContextPrecisionConfig config =
            ContextPrecisionMetric.ContextPrecisionConfig.builder().language("ko").build();
